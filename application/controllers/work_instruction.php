...
<?php
class Work_instruction extends My_Controller {
	...
	{	
		...
		$data['title_view'] = "Work Instruction";
		
		$this->load->view('includes/header', $data);
		$this->load->view('includes/top_nav');
		$this->load->view('includes/sidebar');
		$this->load->view('Work_instruction/index');
	...
	public function insert(){
		...
		$procedure->procedure_type          = "work instruction";
		...
		if(count($check_procedure) > 0){
			$result = false;
			$msg = "Work Instruction already exist.";
		}	
		else{
			...
			if(count($step) > 0){
				...
				$result = true;
				$msg = "Work Instruction submitted successfully.";
	...
	public function  update(){
		...
		if(count($check_procedure) > 0){
			...
			$result = true;
			$msg = "Work Instruction updated successfully.";
			...
			$edit_proc->edit_type           = "work instruction";
			...
			$check_edit = $edit_proc->search(array("procedure_id"=>$procedure_id));
			
			if(count($check_edit) > 0){
				$result = false;
				$msg = "The work instruction you have requested to edit is already submitted.";
			}
			else{
			   // save temp
				$edit_proc->save();
				...
				$result = true;
				$msg = "The work instruction you have requested to edit is successfully submitted.";
			}
	...
	public function load_list(){
		...
		$procedure->db->where("procedure_type = 'work instruction'");
	...
	public function load_list_for_approval(){
		...
		foreach($list as $l){
		  $l_pending[] = array(
		  					...
		  					"desc"          =>   "New",
		  					"date_created"  =>   date_format(date_create($l->procedure_date_modified),"F d, Y h:i A"),
		  					"action"        =>   "<button onclick=\"delete_procedure({$l->procedure_id},'new')\" title='Delete' class='btn btn-danger btn-xs flat'><i class='fa fa-times'></i> Delete</button> <button onclick=\"edit_procedure({$l->procedure_id})\" title='Edit' class='btn btn-info btn-xs flat'><i class='fa fa-edit'></i> Edit</button>"
		  					);	
		}
		...
		$procedure->db->where("edit_type = 'work instruction'");
		$list = $proc_edit->get();
		...
		$procedure->db->where("delete_type = 'work instruction'");
	...
	public function delete_procedure(){
		...
		if($type == "new"){
			...
			$result = true;
			$msg = "Work instruction deleted successfully.";
		}
		elseif($type == "edit"){
			$proc_edit = new Procedure_edit;
			...
			$result = true;
			$msg = "The work instruction you have requested to edit is successfully deleted";
		}
		elseif($type == "delete"){
			$proc_delete = new Procedure_delete;
			...
			$result = true;
			$msg = "The work instruction you have requested to delete is successfully deleted";
		}
	...
	public function request_delete(){
		...
		$proc_del->delete_type = "work instruction";

		$check_procedure = $proc_del->search(array("procedure_id"=>$procedure_id));

		if(count($check_procedure) > 0){
			$result = false;
			$msg = "The work instruction you have requested to delete is already submitted.";
		}
		else{
			...
			$result = true;
			$msg = "The work instruction you have requested to delete is submitted successfully.";
		}
