...
<script type="text/javascript">
	$(".content-header small").html("<a href=\"<?php echo base_url('mail_dispatch') ?>\" class='btn btn-xs btn-primary flat'><i class='fa fa-arrow-left'></i> Back to Mail</a>");
</script>
...
<section class="content">
	...
	<div class="view_email_container">
		...
		<div class="row">
			<div class="col-lg-10 clearfix">
				...
				<div style="padding-left:50px;">
					...
					<form id="form_dispatch" method="post" action="<?php echo base_url('mail_dispatch/dispatch'); ?>" enctype="multipart/form-data">
        ...
        // <select id="select-to"></select>
			...
			var projectUserIds = $('#select-to').selectize();
	        var selectize = projectUserIds[0].selectize;
	        selectize.setValue([<?php foreach ($array as $key => $value){ echo "'".$value['email']."',"; } ?>]);
						...
						<input class="hide" value="<?php echo $mail_id ?>" type="text" name="mail_id">
						<input class="hide" value="<?php echo $subject ?>" type="text" name="mail_subject">
						<textarea class="hide" name="mail_content"><?php echo $content ?></textarea>
						<input class="hide" type="hidden" name="attachments" value="<?php echo htmlentities(serialize($att_array)); ?>">
						</div>
						<div class="form-group">
							<button type="submit" class="btn btn-success flat"><i class="fa fa-send"></i> Dispatch</button>
						</div>
					</form>
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("form#form_dispatch").on("submit", function(e){
			...
			$.ajax({
	          url: "<?php echo base_url('mail_dispatch/dispatch') ?>",
	          type: "POST",            
