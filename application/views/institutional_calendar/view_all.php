...
load_dep_calendars();
  ...
  function load_calendars(){
    var display = "";
    $.ajax({
      url: "<?php echo base_url('calendar/load_calendars') ?>",
      dataType: "json",
  ...
  function load_dep_calendars(){
    var display = "";
    $.ajax({
      url: "<?php echo base_url('calendar/load_department_calendars') ?>",
      dataType: "json",
      success: function(data){
        if(data.length > 0){
          for(var i=0; i<data.length;i++){
      ...
      $(function () {
        ...
        $('#calendar').fullCalendar({
          ...
          events: "<?php echo base_url('calendar/load_events_in_calendar') ?>",
          ...
          dayClick: function(date, jsEvent, view) {
            ...
            location.href="<?php echo base_url('calendar/create_event').'/'?>"+Base64.encode(selectedDate);
          }
    ...
    function create_event(calendar_id){
      location.href="<?php echo base_url('calendar/create_event').'/'?>"+Base64.encode(calendar_id);
    }
    function view_event(event_id){
      location.href="<?php echo base_url('calendar/view_event').'/'?>"+Base64.encode(event_id);
    }
    function view_dep_event(event_id){
      location.href="<?php echo base_url('calendar/view_dep_event').'/'?>"+Base64.encode(event_id);
    }
    function edit_event(event_id){
      location.href="<?php echo base_url('calendar/edit_event').'/'?>"+Base64.encode(event_id);
    }
        ...
        success: function(data){
          ...
          for(var i = 0;i<data['list'].length;i++){
              event_list.fnAddData([
                                  ...
                                  "<span class='pull-right'><a href='#' onclick=\"view_event('"+data['list'][i]['event_id']+"')\"><i class='fa fa-eye'></i> View</a></span>"
                                  ]); 
          }
    ...
    function get_dep_events(){
      $.ajax({
        url: "<?php echo base_url('calendar/get_dep_events') ?>",
    ...
    function delete_calendar(){
      $.ajax({
        url: "<?php echo base_url('calendar/delete_calendar') ?>",
        data: {calendar_id : content_menu_selected_id},
        type: "GET",
        dataType: "json",
        success: function(data){
    ...
    function change_calendar(calendar_id, calendar_name){
      ...
      $('#calendar').fullCalendar('removeEventSource',"<?php echo base_url('calendar/load_events_in_calendar') ?>");
       $('#calendar').fullCalendar('removeEventSource',"<?php echo base_url('calendar/load_dep_events_in_calendar').'/' ?>"+prev_dep_cal);
      $('#calendar').fullCalendar('removeEventSource',"<?php echo base_url('calendar/load_events_in_calendar').'/' ?>"+prev_cal);
      $('#calendar').fullCalendar('addEventSource',"<?php echo base_url('calendar/load_events_in_calendar').'/' ?>"+calendar_id); 
    ...
    function change_dep_calendar(dep_id, calendar_name){
      ...
      $('#calendar').fullCalendar('removeEventSource',"<?php echo base_url('calendar/load_events_in_calendar').'/' ?>"+prev_cal);
      $('#calendar').fullCalendar('removeEventSource',"<?php echo base_url('calendar/load_dep_events_in_calendar').'/' ?>"+prev_dep_cal);
      $('#calendar').fullCalendar('addEventSource',"<?php echo base_url('calendar/load_dep_events_in_calendar').'/' ?>"+dep_id); 
      prev_dep_cal = dep_id;
    }
    function view_all_calendar(){
      ...
      $('#calendar').fullCalendar('removeEventSource',"<?php echo base_url('calendar/load_events_in_calendar') ?>");
       $('#calendar').fullCalendar('removeEventSource',"<?php echo base_url('calendar/load_dep_events_in_calendar').'/' ?>"+prev_dep_cal);
      $('#calendar').fullCalendar('removeEventSource',"<?php echo base_url('calendar/load_events_in_calendar').'/' ?>"+prev_cal);
      $('#calendar').fullCalendar('addEventSource',"<?php echo base_url('calendar/load_events_in_calendar') ?>");
    }
    ...
    function search_event(key){
      ...
      if(key.length == 0 || key.trim() == ""){
        ...
        $.ajax({
          url: "<?php echo base_url('calendar/search_event') ?>",
          data: {search : key},
          type: "GET",
          dataType: "html",
          success: function(data){
