		...
		<div class="col-lg-12">
			<div>
			  ...
			  <ul class="nav nav-tabs" role="tablist">
			    <li id="tab_create" role="presentation" class="active"><a href="#create" aria-controls="create" role="tab" data-toggle="tab"><i class="fa fa-edit"></i> Create Work Instruction</a></li>
			    <li id="tab_list" role="presentation"><a href="#list" aria-controls="list" role="tab" data-toggle="tab"><i class="fa fa-list-ol"></i> Work Instruction List</a></li>
			    <li role="presentation"><a href="#approval" aria-controls="approval" role="tab" data-toggle="tab"><i class="fa fa-check"></i> Work Instruction for Approval</a></li>
			  </ul>

			  <!-- Tab panes -->
			  <div class="tab-content" style="padding:20px;background:#FFF">
			    <div role="tabpanel" class="tab-pane active" id="create">
			    	<form id="form_create_procedure" method="post" action="<?php echo base_url("work_instruction/update"); ?>">
			    	<div class="row">

			    		<div class="col-lg-7">

			    			<div class="form-group">
			    				<button class="btn btn-success flat" type="submit"><i class="fa fa-save"></i> Save Work Instruction</button>
			    				<button id="btn-reset" class="btn btn-danger flat" type="reset"><i class="fa fa-times"></i> Cancel</button>
			    			</div>

			    			<div class="form-group hide">
			    				<b>Work Instruction ID</b>
			    				<div class="form-group">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">WHY DELETE WORK INSTRUCTION ?</h4>
      </div>
      <form id="form-submit-request" method="post" action="<?php echo base_url('work_instruction/request_delete') ?>">
...
<script type="text/javascript">
	...
	$(document).ready(function(){
	  ...
	  $("button#btn-add-step").click(function(e){
			$.ajax({
				url: "<?php echo base_url('work_instruction/add_step') ?>",
				dataType: "html",
      ...
      $('#form_create_procedure').bootstrapValidator({
          ...
          if($("#steps_container").html().trim() != ""){
          	...
          	if(procedure_for_edit == 0){
          		url = "<?php echo base_url('work_instruction/insert') ?>";
          	}else if(procedure_for_edit != 0){
          		url = "<?php echo base_url('work_instruction/update') ?>";
          	}
          	$.ajax({
              url: url,
              type: "POST",            
          	...
          	new PNotify({
                          title: 'Error',
                          text: 'Unable to save work instruction. Please add step.',
                          type: 'error',
                          icon: 'fa fa-times'
                    });
          }
	...
	function load_list_approval(stat){
		$.ajax({
			url: "<?php echo base_url('work_instruction/load_list_for_approval') ?>",
			data: {status : stat},
			dataType: "json",
			type: "GET",
			success: function(data){
	...
	function delete_procedure(id, type){
		$.ajax({
			url: "<?php echo base_url('work_instruction/delete_procedure') ?>",
  ...
  $(document).ready(function(e){
    $('#form-submit-request').bootstrapValidator({
          ...
          $.ajax({
              url: "<?php echo base_url('work_instruction/request_delete') ?>",
              type: "POST",            
              data: new FormData(this),
              dataType: "json",
              contentType: false,      
	...
	function load_list_procedure(stat){
		$.ajax({
			url: "<?php echo base_url('work_instruction/load_list') ?>",
			data: {status : stat},
			dataType: "json",
			type: "GET",
			success: function(data){
	...
	function edit_procedure(procedure_id){
		...
		$.ajax({
			url: "<?php echo base_url('work_instruction/edit') ?>",
